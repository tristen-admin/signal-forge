...
*Fixed 7/30/26 (2-hour quick-fix batch, commits a96179c/32d597d, then the Ourevos/Anorith build below): Legend Board's currency icon, Kessuae's fake discard text, Keawe Kel'rua's unbacked duel-count condition, the Waltz Twins race-vs-Link-group text (all 4 printings) — see [[project_signalforge_quickfix_batch_073026]] in memory for exact diffs.*
...
**✅ Both CALLED entries below now have real mechanics, built 7/30/26 (owner: "Yes build") — resolved, not a text rewrite:**
- **Ourevos, the Golden Dragon**: `add` corrected 2→5 to match its own printed "+5"; added `draw:1`; added a genuinely new `applyCalled()` primitive, `duplicateSelfToSupport` — pushes a shallow copy of the support into the other rear-guard slot if one is open (same shape as the existing `conjureTopToSupport`, just sourcing the card from itself instead of the deck).
- **Anorith Keeling**: added `draw:2` and `chargeGain:1` (both pre-existing primitives, trivial to wire). Added a new primitive, `returnSelfToDeck` — nulls the card's own `rearGuards` slot in place (not a splice, so the live `forEach` mid-iteration doesn't skip whatever's in the other slot) and pushes+shuffles it into `drawPile`.
Both verified live: forced each into a support slot mid-duel and confirmed the log lines, the power math, the second Ourevos landing in the open slot, and Anorith leaving `rearGuards` and reappearing in `drawPile`.
